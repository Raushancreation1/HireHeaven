...
export const updateProfilePic = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    const [updatedUser] = await sql`
    UPDATE users SET profile_pic = ${uploadResult.url}, profile_pic_public_id = ${uploadResult.public_id} WHERE user_id = ${user.user_id} RETURNING user_id, name, profile_pic, profile_pic_public_id;
`;
...
export const addSkillToUser = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    try {
        ...
        const insertionResult =
            await sql`INSERT INTO user_skills (user_id, skill_id) VALUES (${userId}, ${skillId}) NO CONFLICT (user_id, skill_id) DO NOTHING RETURNING user_id`;
    ...
    res.json({
        message: `Skill ${skillName.trim()} is added successfilly`,
    });
}
);
